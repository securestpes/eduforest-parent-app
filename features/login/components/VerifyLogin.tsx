...
import { mapParentMeToUser, ParentProfileService } from '../../profile/services/ParentProfileService';
import { registerParentPushToken } from '../../../src/services/push';
...
const LocalOTPInputs = React.forwardRef(
  (
    ...
    const handleKeyPress = ({ nativeEvent }: { nativeEvent: { key: string } }, idx: number) => {
...
export const VerifyLogin: React.FC<Props> = ({ navigation, route }) => {
  ...
  const finalizeBackendFromFirebaseUser = useCallback(async () => {
    ...
    try {
      ...
      if (SHOW_FIREBASE_OTP_VERIFY_DEBUG) {
        console.log('[VerifyLogin] parent verify-otp', { mobile, hasToken: Boolean(idToken) });
      }
      ...
      if (response.status && response.data && typeof response.data === 'object') {
        ...
        if (data.accessToken) {
          await AsyncStorage.setItem(localStorageKeys.ACCESS_TOKEN, data.accessToken);
          ...
          if (userResponse?.status && userResponse.data) {
            dispatch(loginSuccess(mapParentMeToUser(userResponse.data as Record<string, unknown>)));
      ...
      loginCompletedRef.current = false;
      if (SHOW_FIREBASE_OTP_VERIFY_DEBUG) {
        console.log('[VerifyLogin] finalize error', error instanceof Error ? error.message : error);
      }
  ...
  useEffect(() => {
    ...
    const run = async () => {
      for (let i = 0; i < 25; i++) {
        ...
        if (
          user &&
          firebasePhoneMatchesIndiaLocal(user.phoneNumber, route.params.mobileNumber)
        ) {
  ...
  useEffect(() => {
    ...
    const unsub = auth().onAuthStateChanged((user) => {
      ...
      if (!firebasePhoneMatchesIndiaLocal(user.phoneNumber, route.params.mobileNumber)) {
        return;
      }
  ...
  const onPressOtpVerification = async () => {
    ...
    try {
      const fbResponse = await AuthService.firebaseVerifyOtp(verificationId, otp);
  ...
  const onPressResendOtp = async () => {
    ...
    try {
      const confirmation = await AuthService.firebaseLoginWithMobile(route.params.mobileNumber, true);
          ...
          >
            <View style={[appStyles.pageHPadding, { marginTop: 60 }]}>
              ...
              <View style={{ marginTop: 60 }}>
                <Text style={styles.verifyLoginHeading}>{t('verifyOtp.title')}</Text>
                <Text style={styles.verifyLoginSubHeading}>{t('verifyOtp.subtitle')}</Text>
              </View>
              <View style={styles.mobileNumberContainer}>
                <Text style={styles.mobileNumberText}>+91-{route.params.mobileNumber}</Text>
                <Link onPress={onPressChangeMobileNumber} style={styles.changeMobiletext}>
                  {t('common.change')}
                </Link>
              </View>
              ...
              <View style={[{ ...appStyles.form }, { marginTop: 20 }]}>
                ...
                />
                {errorMessage ? (
                  <Text style={{ color: theme.colors.error, marginBottom: 10 }}>{errorMessage}</Text>
                ) : null}
                {successMessage ? (
                  <Text style={{ color: theme.colors.success, marginBottom: 10 }}>{successMessage}</Text>
                ) : null}
                ...
                >
                  {canResend ? (
                    ...
                    <Text>
                      {t('verifyOtp.resendIn')} <OtpTimer initial={20} onCountdownEnd={resetCountDown} /> s
                    </Text>
                  )}
                </Link>
              </View>
