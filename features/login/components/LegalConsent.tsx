...
export const LegalConsent: React.FC<LegalConsentProps> = ({ navigation }) => {
  ...
  const handleContinue = async () => {
    ...
    try {
      setSaving(true);
      await AsyncStorage.setItem(localStorageKeys.LEGAL_CONSENT_ACCEPTED, 'true');
      navigation.reset({
        index: 0,
        routes: [{ name: 'Login' }],
      });
    ...
    <SafeAreaView style={appStyles.pageContainer} edges={['bottom']}>
      <ScrollView contentContainerStyle={{ flexGrow: 1, paddingBottom: 24 }}>
        <View style={[appStyles.pageHPadding, { marginTop: 40 }]}>
          ...
          <Text style={styles.loginSubHeading}>{t('legalConsent.subtitle')}</Text>

          <View style={{ marginTop: 24 }}>
            <Text style={{ fontWeight: '600', marginBottom: 8 }}>{t('legalConsent.howWeUseInfo')}</Text>
            <Text style={{ color: theme.colors.secondaryText, marginBottom: 4 }}>
            ...
            <Text style={{ color: theme.colors.secondaryText }}>{'\u2022'} {t('legalConsent.point4')}</Text>
          </View>

          <View style={{ marginTop: 24 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12 }}>
              <Checkbox status={acceptTerms ? 'checked' : 'unchecked'} onPress={() => setAcceptTerms((v) => !v)} />
              <Text style={{ flex: 1 }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <Checkbox status={acceptPrivacy ? 'checked' : 'unchecked'} onPress={() => setAcceptPrivacy((v) => !v)} />
              <Text style={{ flex: 1 }}>
          ...
          <View style={{ marginTop: 32 }}>
            <StyledButton mode="contained" onPress={() => void handleContinue()} disabled={!canContinue} loading={saving}>
              {t('common.continue')}
            </StyledButton>
          </View>
        </View>
