...
export function ParentOnboardingModal({ visible, onComplete }: Props) {
  ...
  const finish = async () => {
    await AsyncStorage.setItem(localStorageKeys.PARENT_ONBOARDING_COMPLETED, 'true');
    onComplete();
  };
      ...
      <View style={styles.backdrop}>
        <View style={[styles.card, { backgroundColor: theme.colors.surface }]}>
          <View style={[styles.iconWrap, { backgroundColor: theme.colors.primaryContainer }]}>
            <MaterialCommunityIcons name={slide.icon} size={40} color={theme.colors.primary} />
          </View>
          <Text variant="headlineSmall" style={{ color: theme.colors.onSurface, fontWeight: '800', textAlign: 'center' }}>
            {t(slide.titleKey)}
          </Text>
          ...
          {!isLast ? (
            <Pressable onPress={() => void finish()} style={{ marginTop: 12, alignItems: 'center' }}>
