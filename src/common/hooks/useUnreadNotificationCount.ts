...
export function useUnreadNotificationCount(): number {
  ...
  const refresh = useCallback(async () => {
    try {
      clearStalePendingPushNotifications();
      const [readIds, stRes] = await Promise.all([
        ...
        students.map(async (student) => {
          try {
            ...
            if (attRes.status && attRes.data?.content)
              rowsMap.set(student.id, attRes.data.content);
  ...
  const scheduleRefreshWithRetry = useCallback(() => {
    ...
    retryTimers.current = RETRY_DELAYS_MS.map((ms) =>
  ...
  useEffect(() => {
    ...
    const onReadsChanged = DeviceEventEmitter.addListener(
      NOTIFICATION_READS_CHANGED_EVENT,
      () => {
        void refresh();
      }
