...
import type { ParentAttendanceRow, ParentSchedule, ParentStudent } from '../services/parent';
import {
...
): { label: string; icon: React.ComponentProps<typeof MaterialCommunityIcons>['name']; bg: string; fg: string } {
  switch (kind) {
    case 'present':
      return {
...
export function TodayHeroCard({ student, schedules, rows }: Props) {
  ...
  return (
    <View style={[styles.card, { backgroundColor: theme.colors.surface, borderColor: theme.colors.outlineVariant }]}>
      ...
      <Text variant="titleMedium" style={{ color: theme.colors.onSurface, fontWeight: '800', marginTop: 4 }}>
        {student.name}
      </Text>

      <View style={[styles.statusRow, { backgroundColor: meta.bg }]}>
        <MaterialCommunityIcons name={meta.icon} size={22} color={meta.fg} />
        <Text variant="titleSmall" style={{ color: meta.fg, fontWeight: '800', marginLeft: 8, flex: 1 }}>
          {meta.label}
        </Text>
      </View>
      ...
      {nextSchedule && kind !== 'no_class' ? (
        <Text variant="bodySmall" style={{ color: theme.colors.onSurfaceVariant, marginTop: 6 }}>
          {t('schedule.classTime')}: {formatScheduleTimeRange(nextSchedule)} · {nextSchedule.batchName}
        </Text>
      ) : null}
    </View>
  );
